...
describe("TaskService", () => {
  ...
  describe("getTasks", () => {
    ...
    it("applies q filter with .or()", async () => {
      const chain = mockChain({ data: [], error: null, count: 0 });
      mockFrom.mockReturnValue(chain);
      await taskService.getTasks({ q: "search-term" });
      expect(chain.or).toHaveBeenCalledWith(
        expect.stringContaining("search-term"),
      );
    });

    it("applies due_before filter with .lte()", async () => {
      const chain = mockChain({ data: [], error: null, count: 0 });
      mockFrom.mockReturnValue(chain);
      await taskService.getTasks({ due_before: "2026-12-31" });
      expect(chain.lte).toHaveBeenCalledWith("due_date", "2026-12-31");
    });

    it("applies due_after filter with .gte()", async () => {
      const chain = mockChain({ data: [], error: null, count: 0 });
      mockFrom.mockReturnValue(chain);
      await taskService.getTasks({ due_after: "2026-01-01" });
      expect(chain.gte).toHaveBeenCalledWith("due_date", "2026-01-01");
    });

    it("applies range when limit and offset are provided", async () => {
      const chain = mockChain({ data: [], error: null, count: 0 });
      mockFrom.mockReturnValue(chain);
      await taskService.getTasks({ limit: 10, offset: 20 });
      expect(chain.range).toHaveBeenCalledWith(20, 29);
    });

    it("applies status_id filter with .eq()", async () => {
      const chain = mockChain({ data: [], error: null, count: 0 });
      mockFrom.mockReturnValue(chain);
      await taskService.getTasks({ status_id: 2 });
      expect(chain.eq).toHaveBeenCalledWith("current_status_id", 2);
    });

    it("applies assigned_to filter with .eq()", async () => {
      const chain = mockChain({ data: [], error: null, count: 0 });
      mockFrom.mockReturnValue(chain);
      await taskService.getTasks({ assigned_to: "u1" });
      expect(chain.eq).toHaveBeenCalledWith("assigned_to", "u1");
    });
  });
  ...
  describe("createTask", () => {
    it("throws when no default status exists", async () => {
      ...
      await expect(taskService.createTask("Title", "desc")).rejects.toThrow(
  ...
  describe("updateTaskStatus", () => {
    it("throws when task does not exist", async () => {
      ...
      mockFrom.mockReturnValue(chain);
      await expect(taskService.updateTaskStatus("no-task", 2)).rejects.toThrow(
        "Task not found",
      );
    });
  ...
  describe("updateTask", () => {
    it("throws VALIDATION_ERROR when no allowed fields provided", async () => {
      await expect(
        taskService.updateTask("t1", { unknown_field: "x" }),
    ...
    it("throws NOT_FOUND when task is not returned after update", async () => {
      ...
      mockFrom.mockReturnValue(chain);
      await expect(
        taskService.updateTask("t1", { title: "New title" }),
