...
describe("AuthController.register", () => {
  ...
  it("calls authService.register and returns 201", async () => {
    ...
    await authController.register(
      { body: { email: "x@y.com", password: "pass1234", name: "X" } },
      res,
      vi.fn(),
    );
  ...
  it("falls back to 400 and REGISTRATION_FAILED when error has no code/status", async () => {
...
describe("AuthController.refreshToken", () => {
  ...
  it("falls back to 401 and INVALID_TOKEN when error has no code/status", async () => {
...
describe("AuthController.getMe", () => {
  ...
  it("falls back to 404 and NOT_FOUND when error has no code/status", async () => {
    authService.getMe.mockRejectedValue(new Error("unexpected"));
    const res = makeRes();
    await authController.getMe({ user: { user_id: "u1" } }, res, vi.fn());
    expect(res.status).toHaveBeenCalledWith(404);
    expect(res.json).toHaveBeenCalledWith(
